...
import com.famas.vlabschemistrymvgr.ui.activities.*
import com.famas.vlabschemistrymvgr.ui.components.ExperimentItemCard
import com.famas.vlabschemistrymvgr.ui.components.LinearProgressLoader
import com.famas.vlabschemistrymvgr.ui.screens.ReportsScreen
import com.famas.vlabschemistrymvgr.ui.theme.VLabsChemistryMVGRTheme
import com.famas.vlabschemistrymvgr.util.*
import com.famas.vlabschemistrymvgr.util.BottomNav.*
import com.famas.vlabschemistrymvgr.util.Constants.ROUTE_EXPERIMENTS
import com.famas.vlabschemistrymvgr.util.experiments.Experiment.*
...
class DashBoardActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val lectUUID = intent.getStringExtra("lect_uuid")!!
                ...
                firestore.collection("_lecturers/$lectUUID/exp_timings")
                    .addSnapshotListener { snapshot, _ ->
                        snapshot?.documents?.forEach {
                            ...
                            this.value[it.id] = map
                            Log.d("myTag", "${it.id} ${it.data}")
                        }
                    }
            }).value
                    ...
                    Scaffold(
                        ...
                        NavHost(
                            ...
                            composable(ROUTE_EXPERIMENTS) {
                                ...
                                {
                                    /*item {
                                        expTimings.forEach { (name, timings) ->
                                            var timer by remember { mutableStateOf("") }
                                            val enable = remember { timings["enable"]?.toDate()!! }
                                            val disable = remember { timings["disable"]?.toDate()!! }
                                            val preTime = remember { Calendar.getInstance().time }

                                            if (preTime.before(disable) && preTime.after(enable)) {
                                                LaunchedEffect(key1 = true) {
                                                    startTimer(disable) {
                                                        timer = it
                                                    }
                                                }
                                                Announcement(message = "$name ends in $timer")
                                            }
                                            else if (preTime.before(enable)) {
                                                LaunchedEffect(key1 = true) {
                                                    startTimer(enable) {
                                                        timer = it
                                                    }
                                                }
                                                Announcement(message = "$name will be enabled in $timer")
                                            }
                                        }
                                    }*/
                                    items(ls) { experiment ->
